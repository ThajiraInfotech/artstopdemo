...
const ProductCard = ({ product, index = 0 }) => {
  ...
  const addToCart = async (e) => {
    ...
    console.log('[PRODUCTCARD DEBUG] addToCart called for product:', product.name, 'ID:', productId);

    // Check if user is authenticated
    const token = localStorage.getItem('artstop_token') || localStorage.getItem('token');
    console.log('[PRODUCTCARD DEBUG] addToCart - Token check:', {
    ...
    if (!token) {
      console.log('[PRODUCTCARD DEBUG] addToCart - No token found, redirecting to login');
      toast({
    ...
    try {
      console.log('[PRODUCTCARD DEBUG] addToCart - Making API call to add product to cart');
      ...
      console.log('[PRODUCTCARD DEBUG] addToCart - API response:', response);

      if (response.success) {
        console.log('[PRODUCTCARD DEBUG] addToCart - Successfully added to cart, triggering cart update event');
        toast({
          title: "Added to Cart",
        ...
        console.log('[PRODUCTCARD DEBUG] addToCart - API response not successful');
      }
    } catch (error) {
      console.error('[PRODUCTCARD DEBUG] Error adding to cart:', error);
      console.log('[PRODUCTCARD DEBUG] addToCart - Error details:', {
