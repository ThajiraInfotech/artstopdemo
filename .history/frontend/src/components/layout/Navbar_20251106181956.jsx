...
const Navbar = () => {
    ...
    const handleAuthUpdate = () => {
      ...
      if (userData && userData !== 'undefined' && userData !== 'null') {
        try {
          setUser(JSON.parse(userData));
          setIsLoggedIn(true);
          // Update cart count when user logs in - add small delay to ensure token is properly stored
          setTimeout(() => {
            fetchCartCount();
          }, 100);
        } catch (error) {
  ...
  const fetchCartCount = async () => {
    ...
    try {
      ...
      // For cart count fetching, don't trigger redirects on auth errors - just set count to 0
      // This prevents login loops when navbar tries to fetch cart count
      if (error.status === 401) {
        console.log('[NAVBAR DEBUG] fetchCartCount - Auth error, setting cart count to 0 without redirect');
        setCartCount(0);
      } else {
