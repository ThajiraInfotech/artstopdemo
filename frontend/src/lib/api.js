...
const decodeJWT = (token) => {
  try {
    ...
    console.error('[API DEBUG] Error decoding JWT:', error);
    return null;
  }
};

// Helper function to handle token expiry - clear session and redirect to login
const handleTokenExpiry = () => {
  console.log('[API DEBUG] Handling token expiry - clearing ALL localStorage and redirecting to login');
  // Clear ALL localStorage to ensure no old tokens remain
  localStorage.clear();
  // Trigger auth update event to update UI
  ...
  if (currentPath !== '/login' && currentPath !== '/signup' && currentPath !== '/auth') {
    console.log('[API DEBUG] Not on auth page, redirecting to login');
    window.location.href = '/login';
  } else {
    console.log('[API DEBUG] Already on auth page, not redirecting');
  }
};
...
const refreshAccessToken = async () => {
  try {
    const refreshToken = localStorage.getItem('artstop_refresh_token');
    if (!refreshToken) {
      console.log('[API DEBUG] No refresh token found - redirecting to login');
      // No refresh token available, redirect to login
      handleTokenExpiry();
      return null;
    }

    console.log('[API DEBUG] Attempting to refresh access token');
    ...
    if (response.token && response.refreshToken) {
      ...
      console.log('[API DEBUG] Token refresh failed - no tokens in response');
      handleTokenExpiry();
      return null;
    }
  } catch (error) {
    console.error('[API DEBUG] Token refresh error:', error);
...
const apiRequest = async (endpoint, options = {}, retryCount = 0) => {
  ...
  try {
    ...
    if (!response.ok) {
      // Check if it's a token expired error and we haven't retried yet
      if (response.status === 401 && data?.message === 'Token expired.' && retryCount === 0 && token) {
        console.log(`[API DEBUG] ${options.method || 'GET'} ${endpoint} - Token expired, attempting refresh`);
        const newToken = await refreshAccessToken();
        if (newToken) {
          // Retry the request with the new token
          console.log(`[API DEBUG] ${options.method || 'GET'} ${endpoint} - Retrying with refreshed token`);
          return apiRequest(endpoint, options, retryCount + 1);
        } else {
          console.log(`[API DEBUG] ${options.method || 'GET'} ${endpoint} - Token refresh failed, proceeding with error`);
        }
      }
